...
def rebuild_csv(csv_file):
    """CSV 파일 완전 재구성"""
    with open(csv_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # 헤더와 첫 데이터가 합쳐진 경우 처리
    if '해설1차시' in content:
        content = content.replace('해설1차시', '해설\n1차시')
    
    lines = content.split('\n')
    
    # 헤더 확인 및 수정
    if lines[0] and not lines[0].startswith('차시;유형;번호;내용;정답;해설'):
        # 헤더가 잘못된 경우
        if '차시;유형' in lines[0]:
            # 헤더 부분만 추출
            header_part = lines[0].split('차시;유형')[0] + '차시;유형;번호;내용;정답;해설'
            data_part = '차시;유형' + lines[0].split('차시;유형', 1)[1] if '차시;유형' in lines[0] else ''
            lines[0] = header_part
            if data_part and '1차시' in data_part:
                lines.insert(1, data_part.split('1차시')[1] if '1차시' in data_part else '')
    ...
    for i, line in enumerate(lines, 1):
        ...
        # 헤더는 수정
        if i == 1:
            fixed_lines.append('차시;유형;번호;내용;정답;해설\n')
            continue
        ...
        번호 = parts[2].strip()
        내용 = parts[3].strip()
        정답 = parts[4].strip() if len(parts) > 4 else ''
        해설 = parts[5].strip() if len(parts) > 5 else ''
        
        # 13차시 2번 퀴즈 수정
        if 차시 == '13차시' and 유형 == '퀴즈' and 번호 == '2':
            if '변동성 구간;X;' in original or '변동성 구간;X' in original:
...
def main():
    ...
    print(f'[진행] CSV 파일 재구성 중: {csv_file}')
    changes = rebuild_csv(csv_file)
