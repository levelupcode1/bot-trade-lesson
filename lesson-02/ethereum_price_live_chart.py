    ...
    def load_initial_data(self):
        ...
        try:
            ...
            if initial_data:
                ...
                print(f"Initial {len(filtered_data)} data points loaded (last 6 hours)")
            else:
            ...
            if current_price:
                ...
                with self.data_lock:
                    ...
                    if (not self.price_history or 
                        ...
                        # Keep only last 6 hours of data (360 data points for 1-minute updates)
                        cutoff_time = now - timedelta(hours=6)
                        self.price_history = [
                ...
                with self.data_lock:
                    if self.price_history:
                        ...
                        if len(dates) > 0:
                            ...
                            max_ticks = 10  # Reduced maximum number of ticks to display
                            
                            # More aggressive interval calculation for 1-minute updates
                            if time_span > 168:  # More than 1 week
                                interval = max(48, int(total_hours / max_ticks))  # 48 hours or more
                                ...
                                interval = max(3, int(total_hours / max_ticks))  # 3 hours or more
                            elif time_span > 2:  # More than 2 hours
                                interval = max(1, int(total_hours / max_ticks))  # 1 hour or more
                            else:  # Less than 2 hours
                                interval = max(1, int(total_hours / max_ticks))  # 30 minutes or more
                            
                            # Additional safety check - ensure interval is at least 1
                            if interval < 1:
                                interval = 1
                            
                            # Use more aggressive locator for high-frequency data
                            if len(dates) > 1000:  # If we have more than 1000 data points
                                # Use day locator for very long periods
                                if time_span > 24:
                                    self.ax.xaxis.set_major_locator(mdates.DayLocator(interval=max(1, int(time_span / 24 / max_ticks))))
        ...
        with self.data_lock:
            ...
            if len(dates) > 0:
                ...
                if len(dates) > 1000:  # If we have more than 1000 data points
                    # Use day locator for very long periods
                    if time_span > 24:
                        ax.xaxis.set_major_locator(mdates.DayLocator(interval=max(1, int(time_span / 24 / max_ticks))))
