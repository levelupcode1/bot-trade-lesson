...
def create_excel(csv_file):
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=1, max_row=ws.max_row):
            ...
            if cell_b.value and str(cell_b.value).isdigit():
                if cell_b:
...
def main():
    ...
    print(f'[1단계] 세미콜론 -> 쉼표 변경: {len(changes)}개')
    
    # 2. 엑셀 파일 생성
    print('[2단계] 엑셀 파일 생성 중...')
    try:
        excel_file, sheet_count = create_excel(csv_file)
        print(f'[성공] 엑셀 파일 생성 완료: {excel_file}')
        print(f'       총 {sheet_count}개 차시 시트 생성됨')
